...
const TypingEngine = () => {
  ...
  const handleInput = (e) => {
    ...
    const lastChar = inputValue.slice(-1); // La última letra escrita
    ...
    <div className="relative w-full max-w-4xl mx-auto p-10 bg-[#323437] rounded-xl overflow-hidden">
      <div className="flex flex-wrap font-mono text-2xl leading-relaxed tracking-wider select-none">
        {chars.map((item, index) => {
          ...
          if (item.status === "correct") colorClass = "text-green-400"; // Blanco
          if (item.status === "incorrect") colorClass = "text-[#ca4754]"; // Rojo
          ...
          return (
            <span
              ...
              className={`relative inline-block font-mono text-2xl ${colorClass} ${
                index === currentIndex
                  ? "after:content-[''] after:absolute after:left-0 after:top-0 after:w-[2px] after:h-full after:bg-[#e2b714] after:animate-pulse"
                  : ""
              }`}
